...
class GraphAutoEncoder(torch.nn.Module):
    ...
    def __init__(self, embed_layer, encoder, reduced_emebdding_sz=64):
        super(GraphAutoEncoder, self).__init__()
        self.embed_layer = embed_layer
        self.encoder = encoder
        self.projection_layer = torch.nn.Linear(2*self.encoder.hidden_size, reduced_emebdding_sz, bias=False)
        self.decoder = GraphDecoder(2*self.encoder.hidden_size, 4*self.encoder.hidden_size, self.encoder.hidden_size)
